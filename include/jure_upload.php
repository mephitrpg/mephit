...
class jure_upload{
	var $k,$files,$method,$couldBeValid;	// $couldBeValid serve nel caso ( !empty_error && count($this->file)==0 ) del metodo isValid()
		...
		for($i=0;$i<count($q);$i++){
			switch($q){
				case"images_all":
					...
					// openoffice
					$this->files[$this->k][allow][]="sxd";	// disegno
				case"images":
				...
				case"docs":
					...
					// openoffice
					$this->files[$this->k][allow][]="sxc";	// foglio elettronico
					$this->files[$this->k][allow][]="sxw";	// testo
					$this->files[$this->k][allow][]="sxi";	// presentazione
				break;
				case"archives_all":
					$this->files[$this->k][allow][]=".tar.gz";	$this->files[$this->k][allow][]=".tar.bz2";
	...
	function addFile($fieldName,$uploadPath/*[, allow] [, empty_error]*/){
		...
		if( isset($_FILES[$fieldName]) ){
			...
			if( $allow_empty && !$this->couldBeValid )$this->couldBeValid=true;
			if(is_array($_FILES[$fieldName][name])){
				...
				if( $allow_empty || $_FILES[$fieldName][error]==0 ){
					...
					$this->files[$this->k][name]=stripslashes($this->files[$this->k][name]);
...
//			print_r($this->files);
		...
		for($i=0;$i<count($this->files);$i++){
			...
			if($this->files[$i][error]==0){
				if(is_uploaded_file($this->files[$i][tmp_name])){
					$ext=strtolower(substr($this->files[$i][name],strrpos($this->files[$i][name],".")+1,strlen($this->files[$i][name])-strrpos($this->files[$i][name],".")-1));
					if($ext=="gz"||$ext=="bz2"){
						$ext2=strtolower(substr($this->files[$i][name],0,strlen($this->files[$i][name])-strlen($ext)-1));
						$ext2=strtolower(substr($this->files[$i][name],strrpos($this->files[$i][name],".")+1,strlen($this->files[$i][name])-strrpos($this->files[$i][name],".")-1));
						if($ext2=="tar")$ext="tar.".$ext;
					}
				...
				switch($this->files[$i][error]){
					...
					case 2:
						...
						$this->files[$i][upMessage]="ERRORE: La dimensione massima del file &egrave; ".($_POST["MAX_FILE_SIZE"]/1024)." Kb";
					break;
					case 3:
						// UPLOAD_ERR_PARTIAL
	...
	function isValid(){
		if(count($this->files)==0)return $this->couldBeValid;
